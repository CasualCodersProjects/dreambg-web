...
serve(async (req: Request) => {
  ...
  try {
    ...
    const event = await stripe.webhooks.constructEventAsync(body, sig, endpointSecret, undefined, cryptoProvider);
    ...
    switch(event.type) {
      ...
      case 'invoice.paid':
        ...
        const data = customerData?.subscribed_on ? {subscription: 'pro', last_paid: new Date()} : {subscription: 'pro', last_paid: new Date(), subscribed_on: new Date()};
